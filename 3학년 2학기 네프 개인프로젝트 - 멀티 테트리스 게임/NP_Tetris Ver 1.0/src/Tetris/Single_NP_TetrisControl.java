...
public class Single_NP_TetrisControl extends JPanel implements Runnable {
	...
	Single_NP_Tetrimino tetrimino; // '현재 떨어지고 있는' 테트리미노
	Single_NP_Tetrimino tetriminoSaved;// 테트리미노 킵핑(저장)
	...
	Single_NP_Tetrimino[] tetrisQueue; // 테트리스 대기목록 tetrisQueue[3]
	...
	public Single_NP_TetrisControl() {
		// TODO Auto-generated constructor stub
		addKeyListener(new MyKeyListener());
		setFocusable(true);

		tetrimino = new Single_NP_Tetrimino(); // 전역변수에 초기값 설정
		tetriminoSaved = new Single_NP_Tetrimino();// 전역변수에 초기값 설정
		...
		tetrisQueue = new Single_NP_Tetrimino[3];// 테트리스 대기목록
	...
	class MyKeyListener extends KeyAdapter {
		public void keyPressed(KeyEvent e) {
			...
			if (!isGameOver)// 게임이 돌아가고 있다면,
			{
				...
				System.out.println(" 눌린 키값은 : " + keyCode);
			}
			if (!tetrimino.getIsLocked()) // 테트리미노가 잠겨있지 않다면,
			{
//				// 일시정지
//				if (keyCode == ENTER && !(isPauseGame) && !(isGameOver)) {
//					isPauseGame = true;
//
//				} else if (keyCode == ENTER && isPauseGame && !(isGameOver)) {
//					isPauseGame = false;
//				}
				// 그외 상하좌우 이동 및 회전
//				else 
...
//				else if ((keyCode == 'h' || keyCode == 'H')) // Hidden																						// RIGHT
//				{
////					isGameOver = true;
//				} 
			}
	...
	public void GameSeting() {
		...
		for (int i = 0; i < maxQueue; i++) {
			tetrisQueue[i] = new Single_NP_Tetrimino();
		}
	...
	public void tetrisNew() {
		...
		tetrisQueue[maxQueue - 1] = new Single_NP_Tetrimino();
	}
	...
	public void BackGround(Graphics g) {
		...
		g.drawString("진행시간 :" + ((endTime - startTime) / 1000) + "초", 210, 30);
		g.drawString("NEXT", 210, 80);
		g.drawString("KEEPING", 210, 320);
		g.drawString("LEVEL   :" + level, 350, 320);// 레벨
		g.drawString("LINE      :" + totalBreakLine, 350, 350);// 지금까지 깬 줄수
		g.drawString("SCORE :" + score, 350, 380);// 스코어
	}
	...
	public void tetrisSave() {
		Single_NP_Tetrimino temp = tetriminoSaved;// 기존에 저장된 테트리미노를 임시변수로 기억한다.
	...
	public void renderGameOver(Graphics g) {
		...
		g.setColor(colors[8]);
		g.setFont(f);
		int x = ((gridWidth*rectWidth)/2)-80;
		int y = ((gridHeight*rectHeight)/2);
