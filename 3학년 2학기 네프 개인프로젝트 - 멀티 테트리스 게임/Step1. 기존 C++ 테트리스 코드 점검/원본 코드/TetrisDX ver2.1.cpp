	...
	while (1)
	{
		...
		if (!tetrimino->isLocked())//테트리미노가 잠겨있지 않다면,
		{
			...
			if (keyCode == ENTER && !(PauseGame) && !(gameOver))
			{
				...
				gotoxy(24, 2);cout << "게임 일시 정지";
				PauseGame = true;
			}
			else if (keyCode == ENTER && PauseGame && !(gameOver))
			{
				SetColor(WHITE, BLACK);
				gotoxy(24, 2); cout << "              ";
				PauseGame = false;
...
#define BGM1 "D:\\3학년 2학기 [2017]\\네트워크 프로그래밍 [전선]\\개인프로젝트\\Step1. 기존 C++ 테트리스 코드 점검\\01-title.wav"
#define BGM2 "D:\\3학년 2학기 [2017]\\네트워크 프로그래밍 [전선]\\개인프로젝트\\Step1. 기존 C++ 테트리스 코드 점검\\02-LOGINSKA.wav"
#define BGM3 "D:\\3학년 2학기 [2017]\\네트워크 프로그래밍 [전선]\\개인프로젝트\\Step1. 기존 C++ 테트리스 코드 점검\\03-BRADINSKY.wav"
#define BGM4 "D:\\3학년 2학기 [2017]\\네트워크 프로그래밍 [전선]\\개인프로젝트\\Step1. 기존 C++ 테트리스 코드 점검\\04-KARINKA.wav"
#define BGM2 "D:\\3학년 2학기 [2017]\\네트워크 프로그래밍 [전선]\\개인프로젝트\\Step1. 기존 C++ 테트리스 코드 점검\\05-TROIKA.wav"
...
int main(int argc, char* argv[])
{
	...
	{
		//게임 처음 화면
		PlaySound(TEXT(BGM1), NULL, SND_FILENAME | SND_ASYNC | SND_LOOP);
...
void GameRanking()
{
	...
	if (!ifs) // if (ifs == NULL)//파일이 존재하지 않는다면,
	{
...
void GameSeting()
{
	PlaySound(TEXT(BGM2), NULL, SND_FILENAME | SND_ASYNC | SND_LOOP);
	system("cls");
	SetColor(WHITE, BLACK);
