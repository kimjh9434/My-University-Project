...
public class MainFrame extends JFrame {
	...
	public MainFrame(FileManager manager, Specification s) {
			...
			public void actionPerformed(ActionEvent e){
				...
				if(d != null&&(!ifList.isSelectionEmpty())){
					if(d.getLevel()==2){
						int categoryIndex = d.getParent().getParent().getIndex(d.getParent());
						int representativeIndex = d.getParent().getIndex(d);
						spec.getCategory(categoryIndex).getRepresentativeValue(representativeIndex).delIfProperty(propertyList.getSelectedIndex());
						propertyList.updateUI();
						ifList.updateUI();
					}
				}
			...
			public void actionPerformed(ActionEvent e){
				if(tree.getLastSelectedPathComponent()!=null)
				{
					...
					case 1 :
						...
						spec.getCategory(categoryIndex).addRepresentativeValue(s);
						expandAllNodes(tree,0,tree.getRowCount());
						tree.updateUI();
						break;
					default :
		...
		editBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				if(tree.getLastSelectedPathComponent()!=null)
				{
					...
					String s;
					if(d.getLevel()==1){
						s = JOptionPane.showInputDialog(null, "새로운 카테고리 이름 입력", "카테고리 편집", 0);
						d.setUserObject(new DefaultMutableTreeNode(s));
						tree.updateUI();
					}else if(d.getLevel()==2){
						s = JOptionPane.showInputDialog(null, "새로운 values 이름 입력", "values 편집", 0);
						d.setUserObject(new DefaultMutableTreeNode(s));
						tree.updateUI();
					}else{
					}
		...
		menuItem2.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e){
				...
				if(ret==0){
					...
					JOptionPane.showMessageDialog(null, "파일 형식이 잘못되었습니다! 다시 시도하세요!","Fail",JOptionPane.WARNING_MESSAGE);
				}
			}
		});
		menu1.add(menuItem2);
		...
		JMenuItem menuItem4 = new JMenuItem("시험용");
		menuItem4.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {
				spec.getCategory(0).getRepresentativeValue(0).addProperty("제발");	
			}
		});
		menu1.add(menuItem4);
